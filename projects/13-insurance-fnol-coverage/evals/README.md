# `13-insurance-fnol-coverage/evals/`: golden set and scores
...
| File | What it does |
|---|---|
| [`golden.jsonl`](golden.jsonl) | Golden set: 22 cases, one JSON object per line with `id`, `input` and `expect`. Examples: `sudden-water-tx-2023-paid`, `seepage-21d-2023-edition-excluded`, `seepage-21d-2019-edition-covered`, `mold-sublimit-texas`, ... |
...
```bash
python -m evals --project 13             # run the cases, refresh scores.json
python -m evals --project 13 --no-write  # gate only (what CI does)
python -m evals --project 13 -v          # list failing cases
```

Each case is executed by `fnol.eval_suite:run_case` in [`fnol/eval_suite.py`](../fnol/eval_suite.py).
