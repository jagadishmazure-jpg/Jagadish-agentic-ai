# `15-banking-credit-memo/evals/`: golden set and scores
...
| File | What it does |
|---|---|
| [`golden.jsonl`](golden.jsonl) | Golden set: 18 cases, one JSON object per line with `id`, `input` and `expect`. Examples: `approve-and-book-dual-control`, `ownership-as-of-2025`, `undisclosed-trust-stops-kyc`, `screening-hit-stops`, ... |
...
```bash
python -m evals --project 15             # run the cases, refresh scores.json
python -m evals --project 15 --no-write  # gate only (what CI does)
python -m evals --project 15 -v          # list failing cases
```

Each case is executed by `credit_memo.eval_suite:run_case` in [`credit_memo/eval_suite.py`](../credit_memo/eval_suite.py).
