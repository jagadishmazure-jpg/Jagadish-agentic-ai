# `18-logistics-exception-agent/evals/`: golden set and scores
...
| File | What it does |
|---|---|
| [`golden.jsonl`](golden.jsonl) | Golden set: 21 cases, one JSON object per line with `id`, `input` and `expect`. Examples: `track-fresh-grounded`, `track-interpolating-model-guarded`, `track-scan-gap-refuses`, `track-scans-stopped-mid-lane`, ... |
...
```bash
python -m evals --project 18             # run the cases, refresh scores.json
python -m evals --project 18 --no-write  # gate only (what CI does)
python -m evals --project 18 -v          # list failing cases
```

Each case is executed by `exception_agent.eval_suite:run_case` in [`exception_agent/eval_suite.py`](../exception_agent/eval_suite.py).
