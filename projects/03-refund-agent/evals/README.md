# `03-refund-agent/evals/`: golden set and scores
...
| File | What it does |
|---|---|
| [`golden.jsonl`](golden.jsonl) | Golden set: 12 cases, one JSON object per line with `id`, `input` and `expect`. Examples: `small-auto-refund`, `large-approved`, `large-rejected`, `large-pending`, ... |
...
```bash
python -m evals --project 03             # run the cases, refresh scores.json
python -m evals --project 03 --no-write  # gate only (what CI does)
python -m evals --project 03 -v          # list failing cases
```

Each case is executed by `refund_agent.eval_suite:run_case` in [`refund_agent/eval_suite.py`](../refund_agent/eval_suite.py).
