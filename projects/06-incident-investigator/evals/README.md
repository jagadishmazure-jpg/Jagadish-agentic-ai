# `06-incident-investigator/evals/`: golden set and scores
...
| File | What it does |
|---|---|
| [`golden.jsonl`](golden.jsonl) | Golden set: 13 cases, one JSON object per line with `id`, `input` and `expect`. Examples: `checkout-rollback-approved`, `checkout-rollback-rejected`, `checkout-pauses-for-approval`, `search-upstream-no-write`, ... |
...
```bash
python -m evals --project 06             # run the cases, refresh scores.json
python -m evals --project 06 --no-write  # gate only (what CI does)
python -m evals --project 06 -v          # list failing cases
```

Each case is executed by `incident_agent.eval_suite:run_case` in [`incident_agent/eval_suite.py`](../incident_agent/eval_suite.py).
