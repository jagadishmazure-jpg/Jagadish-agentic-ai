# `17-automotive-technician-copilot/evals/`: golden set and scores
...
| File | What it does |
|---|---|
| [`golden.jsonl`](golden.jsonl) | Golden set: 17 cases, one JSON object per line with `id`, `input` and `expect`. Examples: `coolant-current-tsb`, `coolant-as-of-before-supersession`, `wrong-version-model-guarded`, `harness-rev-by-year`, ... |
...
```bash
python -m evals --project 17             # run the cases, refresh scores.json
python -m evals --project 17 --no-write  # gate only (what CI does)
python -m evals --project 17 -v          # list failing cases
```

Each case is executed by `tech_copilot.eval_suite:run_case` in [`tech_copilot/eval_suite.py`](../tech_copilot/eval_suite.py).
