# `09-collections-agent/evals/`: golden set and scores
...
| File | What it does |
|---|---|
| [`golden.jsonl`](golden.jsonl) | Golden set: 13 cases, one JSON object per line with `id`, `input` and `expect`. Examples: `approved-plan-sent`, `reviewer-rejects`, `pauses-for-review`, `hardship-referral`, ... |
...
```bash
python -m evals --project 09             # run the cases, refresh scores.json
python -m evals --project 09 --no-write  # gate only (what CI does)
python -m evals --project 09 -v          # list failing cases
```

Each case is executed by `collections_agent.eval_suite:run_case` in [`collections_agent/eval_suite.py`](../collections_agent/eval_suite.py).
